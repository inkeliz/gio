...
type Functions struct {
	...
	*FunctionCaller
	...
	cacheInteger  map[Enum]int
	cacheFloat    map[Enum]float32
	cacheInteger4 map[Enum][4]int
	cacheFloat4   map[Enum][4]float32
...
func NewFunctions(ctx Context, forceES bool) (*Functions, error) {
	f := &Functions{
		...
		cacheInteger:   make(map[Enum]int, 32),
		cacheFloat:     make(map[Enum]float32, 32),
		cacheInteger4:  make(map[Enum][4]int, 32),
		cacheFloat4:    make(map[Enum][4]float32, 32),
	}
...
func (f *Functions) GetInteger(pname Enum) int {
	...
	c, ok := f.cacheInteger[pname]
	if !ok {
		c = f.FunctionCaller.GetInteger(pname)
		f.cacheInteger[pname] = c
	}
	return c
}
func (f *Functions) GetFloat(pname Enum) float32 {
	c, ok := f.cacheFloat[pname]
	if !ok {
		c = f.FunctionCaller.GetFloat(pname)
		f.cacheFloat[pname] = c
	}
	return c
}

func (f *Functions) GetInteger4(pname Enum) [4]int {
	c, ok := f.cacheInteger4[pname]
	if !ok {
		c = f.FunctionCaller.GetInteger4(pname)
		f.cacheInteger4[pname] = c
	}
	return c
}

func (f *Functions) GetFloat4(pname Enum) [4]float32 {
	c, ok := f.cacheFloat4[pname]
	if !ok {
		c = f.FunctionCaller.GetFloat4(pname)
		f.cacheFloat4[pname] = c
	}
	return c
}
