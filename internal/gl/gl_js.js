(() => {
    ...
    let types = {
        ...
        [TypeValue]: {
            ...
            Decoder: function (addr) {
                let id = types[TypeInt32].Decoder(addr)
                return values[id];
            },
        },
        [TypeMethod]: {
    ...
    Object.assign(go.importObject.go, {
        ...
        "gioui.org/internal/gl.newMethod": (sp) => {
        ...
        "gioui.org/internal/gl.free": (sp) => {
            const ref = types[TypeInt64].Decoder(sp);
            valuesPool.push(ref);
        },
        "gioui.org/internal/gl.buffer": (sp) => {
