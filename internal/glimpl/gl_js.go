...
func (f *Functions) GetRenderbufferParameteri(target, pname Enum) int {
	return int(getRenderbufferParameteri(f.Ref, target, pname))
}
func (f *Functions) GetFramebufferAttachmentParameteri(target, attachment, pname Enum) int {
	return int(getFramebufferAttachmentParameteri(f.Ref, target, attachment, pname))
}
func (f *Functions) GetBinding(pname Enum) Object {
	return Object(makeValue(getBinding(f.Ref, pname)))
}
func (f *Functions) GetInteger(pname Enum) int {
	return int(getInteger(f.Ref, pname))
}
func (f *Functions) GetProgrami(p Program, pname Enum) int {
	return int(getProgrami(f.Ref, p, pname))
}
func (f *Functions) GetProgramInfoLog(p Program) string {
	return makeValue(getProgramInfoLog(f.Ref, p)).String()
}
func (f *Functions) GetQueryObjectuiv(query Query, pname Enum) uint {
	return uint(getQueryObjectuiv(f.Ref, query, pname))
}
func (f *Functions) GetShaderi(s Shader, pname Enum) int {
	return int(getShaderi(f.Ref, s, pname))
}
...
func (f *Functions) GetUniformBlockIndex(p Program, name string) uint {
	return uint(getUniformBlockIndex(f.Ref, p, name))
}
...
func initGL(ctx Context) (uint64, uint64)
